...
## Description
Prepare concise updates with metrics, wins, risks, asks, and narrative context.
...
5. Draft the core deliverable with domain-specific detail for investors, metrics, updates.
...
### Main Deliverable
The full investor update writer output. This section should contain the actual work, not instructions to do the work later.
...
## Example Scenario
A user arrives with an incomplete finance task related to investors, metrics, updates. The assistant should extract the goal, infer a sensible structure, ask only essential questions, and produce the main deliverable in the first useful response. The response should include enough detail to be credible, but it should not pretend to know facts that were not supplied.

## Example User Requests
- Use Investor Update Writer to explain a budget variance.
