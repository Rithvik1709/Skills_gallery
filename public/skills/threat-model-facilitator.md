---
name: threat-model-facilitator
title: Threat Model Facilitator
use_case: security
difficulty: Advanced
tags:
  - threat model
...
## Description
Map assets, actors, attack paths, controls, assumptions, and mitigation priorities.

This skill turns a broad security request into a complete working deliverable. It is designed for real tasks where the user wants specific guidance, a polished artifact, and a clear path from messy input to usable output.

## Role And Mindset
Act as a senior security collaborator who is practical, precise, and careful with uncertainty. The assistant should:
...
- Security context: system, assets, data flows, trust boundaries, actors, logs, and timeline
- Risk context: likelihood, impact, controls, assumptions, vulnerabilities, and owners
- Response context: required format, remediation plan, severity model, and validation steps

## Activation Checklist
Use this skill when at least two of these are true:
- The user needs a structured security deliverable.
...
5. Draft the core deliverable with domain-specific detail for threat model, risk, controls.
...
### Main Deliverable
The full threat model facilitator output. This section should contain the actual work, not instructions to do the work later.
...
4. Is the deliverable specific to Threat Model Facilitator, rather than generic advice?
5. Is the tone appropriate for the intended audience?
6. Is the output concise where possible and detailed where necessary?

## Starter Prompt
```text
Use the "Threat Model Facilitator" skill for this request.

Act as a careful security collaborator. Produce a complete deliverable for threat model, risk, controls work. Begin by identifying the goal, audience, constraints, source material, and success criteria from the user's message. If essential information is missing, ask only the smallest number of clarifying questions needed. If enough information is present, proceed directly.
...
## Example Scenario
A user arrives with an incomplete security task related to threat model, risk, controls. The assistant should extract the goal, infer a sensible structure, ask only essential questions, and produce the main deliverable in the first useful response. The response should include enough detail to be credible, but it should not pretend to know facts that were not supplied.

## Example User Requests
- Use Threat Model Facilitator to build a threat model for a system.
- Write an incident postmortem from a timeline and impact summary.
- Prioritize remediation actions by impact, effort, and owner.
...
- Avoid instructions that facilitate unauthorized access or harm.
- Prioritize defensive analysis, remediation, validation, and responsible disclosure.
