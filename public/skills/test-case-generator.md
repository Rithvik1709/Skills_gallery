...
## Description
Create focused unit, integration, and edge-case tests from behavior requirements.
...
5. Draft the core deliverable with domain-specific detail for unit tests, edge cases, coverage.
...
### Main Deliverable
The full test case generator output. This section should contain the actual work, not instructions to do the work later.
...
## Example Scenario
A user arrives with an incomplete developer task related to unit tests, edge cases, coverage. The assistant should extract the goal, infer a sensible structure, ask only essential questions, and produce the main deliverable in the first useful response. The response should include enough detail to be credible, but it should not pretend to know facts that were not supplied.

## Example User Requests
- Apply Test Case Generator to a code diff and list the highest-risk issues.
