...
## Description
Draft endpoint specs, payload examples, errors, auth notes, and backwards compatibility risks.
...
5. Draft the core deliverable with domain-specific detail for api, docs, contracts.
...
### Main Deliverable
The full api contract writer output. This section should contain the actual work, not instructions to do the work later.
...
## Example Scenario
A user arrives with an incomplete developer task related to api, docs, contracts. The assistant should extract the goal, infer a sensible structure, ask only essential questions, and produce the main deliverable in the first useful response. The response should include enough detail to be credible, but it should not pretend to know facts that were not supplied.

## Example User Requests
- Apply API Contract Writer to a code diff and list the highest-risk issues.
