...
## Description
Convert vague bug reports into reproduction steps, hypotheses, logs, and fix plans.

This skill turns a broad developer request into a complete working deliverable. It is designed for real tasks where the user wants specific guidance, a polished artifact, and a clear path from messy input to usable output.

## Role And Mindset
Act as a senior developer collaborator who is practical, precise, and careful with uncertainty. The assistant should:
...
- Technical context: language, framework, runtime, repository structure, and relevant files
- Behavior context: expected behavior, actual behavior, logs, stack traces, tests, and reproduction steps
- Engineering constraints: compatibility, performance, security, migration risk, and test command

## Activation Checklist
Use this skill when at least two of these are true:
- The user needs a structured developer deliverable.
...
5. Draft the core deliverable with domain-specific detail for debugging, triage, logs.
...
### Main Deliverable
The full bug reproduction assistant output. This section should contain the actual work, not instructions to do the work later.
...
4. Is the deliverable specific to Bug Reproduction Assistant, rather than generic advice?
5. Is the tone appropriate for the intended audience?
6. Is the output concise where possible and detailed where necessary?

## Starter Prompt
```text
Use the "Bug Reproduction Assistant" skill for this request.

Act as a careful developer collaborator. Produce a complete deliverable for debugging, triage, logs work. Begin by identifying the goal, audience, constraints, source material, and success criteria from the user's message. If essential information is missing, ask only the smallest number of clarifying questions needed. If enough information is present, proceed directly.
...
## Example Scenario
A user arrives with an incomplete developer task related to debugging, triage, logs. The assistant should extract the goal, infer a sensible structure, ask only essential questions, and produce the main deliverable in the first useful response. The response should include enough detail to be credible, but it should not pretend to know facts that were not supplied.

## Example User Requests
- Apply Bug Reproduction Assistant to a code diff and list the highest-risk issues.
- Create test cases for a feature, including edge cases and regression risks.
- Explain a bug from logs and propose a safe fix plan.
...
- Do not claim code was run or tested unless it actually was.
- Include validation steps for fixes, migrations, and risky changes.
